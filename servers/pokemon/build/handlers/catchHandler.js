...
exports.getCatchHandler = (request, response, { PlayerModel, BattlePokemonModel }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!pokemonEncountering) {
        ...
        response.status(400).json({
            status: "failed",
            msg: "You are not in encounter with any pokemon"
        });
        return;
    }
    if (pokemonEncountering.pokemonName.toLowerCase() !== pokemonName.toLowerCase()) {
        response.set(contentType, applicationJSON);
        response.status(400).json({
            status: "failed",
            msg: "You are already in an encounter with " + pokemonEncountering
        });
        return;
    }
